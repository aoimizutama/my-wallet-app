...
def create_gui():
    ...
    def create_sql(item_name):
        ...
        # データベースに接続
        c = sqlite3.connect("database6.db")
        ...
        if (get == 0 and pay == 0):
            ...
            try:
                ...
                """)
                mid = tuple(maxid) #sqlの戻り値は必ずタプル型かリスト型で定義すること
                if not mid: #タプル型及びリスト型の空地は真偽値で確認できる、if notは空地のときの処理
                    bal = 0 #ここに初期値を代入できればいいのでは？どうする？
                else:
    ...
    def change_sql(item_name):
        ...
        if (get == 0 and pay == 0):
            ...
            try:
                ...
                maxid = c.execute("""
                SELECT bal FROM acc_data
                WHERE id = (SELECT MAX(id) FROM acc_data) - 1
                """)
                bal = maxid.fetchone()[0]
