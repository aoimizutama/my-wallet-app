...
def create_gui():
    ...
    def create_sql(item_name):
        ...
        # データベースに接続
        c = sqlite3.connect("database7.db")
        ...
        if (get == 0 and pay == 0):
            ...
            try:
                ...
                try:
                    #idが最大の行の残金を取得
                    maxid = c.execute("""
                    SELECT bal FROM acc_data
                    WHERE id = (SELECT MAX(id) FROM acc_data)
                    """)
                    bal = maxid.fetchone()[0]
                except:
                    try:
                        #ログインテーブルの現金残を取得
                        login_bal = c.execute("""
                        SELECT cash_bal FROM login
                        """)
                        bal =  login_bal.fetchone()[0]
                    except:
                        messagebox.showerror("エラー","上のメニューから初期設定ボタンをクリックし、初期設定を行ってから登録してください")
        
                # SQLを発行してDBへ登録
                c.execute("""
                INSERT INTO acc_data(acc_date,item_code,get,pay,bal)
                VALUES('{}',{},{},{},{});
                """.format(acc_data,item_code,get,pay,bal+get-pay))
                c.execute("COMMIT;")
                print("登録しました！")
    ...
    def change_sql(item_name):
        ...
        if (get == 0 and pay == 0):
            ...
            try:
                ...
                # SQLを発行してDBへ登録
                c.execute("UPDATE acc_data SET acc_date=?, item_code=?, get=?, pay=?, bal=? WHERE id = ?",
                          (acc_data, item_code, get, pay, bal+get-pay, change_id))
                c.execute("COMMIT;")
                print("更新しました！")
    ...
    frame.pack(fill="x")
    button0 = tk.Button(frame,text="初期設定",command=login_button)
    ...
    # 現金残のラベルとエントリーの設定
    frame6 = tk.Frame(root,padx=30,pady=60)
    frame6.pack(fill="x")
...
def login_gui():
    ...
    def login_sql():
        ...
        # SQLを発行してDBへ登録
        c.execute("""
        INSERT INTO login(user_name,pas,user_id,cash_bal,bank_bal)
        VALUES('{}','{}','{}',{},{});
        """.format(user_name,pas,user_id,cash_bal,bank_bal))
        c.execute("COMMIT;")
        print("設定しました！")
    ...
    frame = tk.Frame(root,bd=2,relief="ridge")
    frame.pack(fill="x")
    button0 = tk.Button(frame,text="初期設定")
    button0.pack(side="left")
...
# 設定のテーブルにデータが登録されてなければ設定画面を表示し、それ以外のときは登録画面を表示
login_bal = tuple(c.execute("""SELECT cash_bal FROM login"""))

if not login_bal: #タプル型及びリスト型の空地は真偽値で確認できる、if notは空地のときの処理
    login_gui()
else:
    create_gui()
