...
<body>
    ...
    <!-- 登録済みの引き出し、預け入れを取り消した現金残、預金残の変数を作成 -->
    ...
    <form action="/kakeiboapp/UpdateServlet2" method="POST">   <!-- 変更ボタンからUpdateServlet2.javaへ　-->
        ...
        <% } %>
        <label>引き出し</label><input type="number" name="getm" value=<%= request.getAttribute("get_money") %>><br>
        <label>預け入れ</label><input type="number" name="outm" value=<%= request.getAttribute("out_money") %>><br>
        <label>メモ</label><input type="text" name="memo" value=<%= request.getAttribute("memo") %>><br>
        <p></p>
        <label>現金残</label><input type="number" name="cash_before" value=<%= request.getAttribute("cash") %> readonly><br>
        <label>預金残</label><input type="number" name="bank_before" value=<%= request.getAttribute("bank") %> readonly><br>
        <input type="hidden" name="cash_after" value=<%= cash_after %>>
        <input type="hidden" name="bank_after" value=<%= bank_after %>><br>
